                ...
                <div id="post" class="tabcontent container-fluid">
                    <div class="text-left">
                        ...
                        <div class="d-flex justify-content-end">
                            <div class="mt-2">
                                <a href="post.php" class="nav-link align-middle px-0 ">
                                    <button class="btn" type="submit" name="switch">Switch to Image</button>
                                </a>
                            </div>
                        </div>
                        <div class="row py-4">
                                            ...
                                            <span>
                                                <form class="d-flex" action="#" method="POST">
                                                    ...
                                                    <div class="px-5 d-flex justify-content-end">
                                                        ...
                                                        <div>
                                                            <button class='btn mx-2 py-1' type='submit' name='postfilter'><i class="fa fa-filter fs-6"></i></button>
                                                        </div>
                                                    </div>
                                                </form>
                                            </span>
                        ...
                        <div class="tableFixHead bg">
                            <table class="table css-serial text-white" style="background-color: #111420;">
                                ...
                                <tbody id="post_search">
                                    ...
                                    if (isset($_POST['free_param'])) {
                                        $srch = $_POST['search'];
                                        $video = ".mp4";
                                        $search = "SELECT p.*,u.user_name,u.id as userid FROM post p inner join users u on p.user_id=u.id where u.user_name LIKE '%" . $srch . "%' OR p.name LIKE '%" . $srch . "%' OR p.description LIKE '%" . $srch . "%'  OR p.tag LIKE '%" . $srch . "%' and gallery Like '%" . $video . "' group by p.id";
                                        $ftcary = searchTable($search);
                                    } elseif (isset($_POST['postfilter'])) {
                                        $from = $_POST['startdate'];
                                        $to = $_POST['enddate'];
                                        $video = ".mp4";
                                        $search = "SELECT p.*,u.user_name,u.id as userid FROM post p inner join users u on p.user_id=u.id where p.created_at BETWEEN '" . $from . "' AND '" . $to . "' and gallery Like '%" . $video . "' group by p.id ";
                                        $ftcary = searchTable($search);
                                    } else {
                                        $video = ".mp4";
                                        $search = "SELECT p.*,u.user_name,u.id as userid FROM post p inner join users u on p.user_id=u.id AND gallery Like '%" . $video . "'group by p.id limit $start_from,$num_per_page";
                                        $ftcary = searchTable($search);
                                    }
                                    ...
                                    while ($pdata = mysqli_fetch_array($ftcary)) {
                                        ...
                                        foreach ($resultImages as $img) {
                                            if ($img != null) {
                                                "<video controls source src='https://beppers.in/apis/gallery/$img' type='video/mp4' height='100' width='100'/>";
                                            }
                                        }
                                    ...
                                    <td class='align-middle'> <video controls source src='https://beppers.in/apis/gallery/$img' type='video/mp4' height='100' width='100'/> </td>
                            ...
                            <div class="text-center">
                                ...
                                if (!(isset($_POST['free_param'])) && !(isset($_POST['postfilter']))) {
                                    ...
                                    if ($page > 1) {
                                        echo "<a href='video_post.php?page=" . ($page - 1) . "' class='btn mx-2'>Previous</a>";
                                    }


                                    for ($i = 1; $i < $total_page; $i++) {
                                        echo "<a href='video_post.php?page=" . $i . "' class='btn mx-1'>$i</a>";
                                    }

                                    if ($i > $page) {
                                        echo "<a href='video_post.php?page=" . ($page + 1) . "' class='btn mx-2'>Next</a>";
                                    }
                                }
                                ?>
                            </div>
